    ...
    def test_create_order(self, token_mock):
        with patch('requests.get') as session:
            session.return_value = MagicMock(
                status_code=200, 
                json=MagicMock(
                    return_value=data.cities_moscow
                )
            )
        ...
        with patch('requests.get') as session:
            session.return_value = MagicMock(
                status_code=200, 
                json=MagicMock(
                    return_value=data.cities_vladivostok
                )
            )
        ...
        sender = self.cdek.get_contact(name="Иванов Иван Иванович", phones=["+79111111111"])
        self.assertEqual("Иванов Иван Иванович", sender.name)
        self.assertIn('+79111111111', [phone.number for phone in sender.phones])

        recipient = self.cdek.get_contact(name="Петров Петр Петрович", phones=["+79222222222"])
        ...
        with patch('requests.post') as session:
            session.return_value = MagicMock(
                status_code=200, 
                json=MagicMock(
                    return_value=data.created_order2
                )
            )
        ...
        self.assertEqual(UUID("72753034-cc78-4472-8e7f-8cebdc04ff86"), order.entity.uuid)
